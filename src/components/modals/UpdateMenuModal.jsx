...
export const UpdateMenuModal = ({ open, setOpen, isRefresh, setIsRefresh, selectMenu }) => {
    ...
    const onSubmit = async (obj) => {
        try {
            ...
            const bodyObj = {
                ...obj,
                imageUrl: imageUrl || selectMenu.imageUrl
            }
            const api = `${BASE_URL}${apiEndPoints.editMenu(selectMenu._id)}`
            const response = await axios.put(api, bodyObj, {
                headers: {
                    Authorization: `Bearer ${Cookies.get("token")} `
                }
            })
